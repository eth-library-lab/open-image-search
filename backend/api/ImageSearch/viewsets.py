...
from ImageSearch.retrieval import knearest_ids
...
def image_search(request):
    ...
    if request.method == 'POST':
        ...
        if serializer.is_valid():
            ...
            # image_features = calculate_image_features(img_path_or_stream) 
            ...
            record_ids = knearest_ids(image_features)
            print("\record_ids: ", record_ids, "\n")
            # record_ids = [1358, 1695, 1839, 2396, 2438, 2963, 5278, 5330, 5337, 5342] # [3,18,19,33,52,236, 308,312,313, 324]
            qry_set = ImageMetadata.objects.all()
            results_metadata = []

            for record_id in record_ids:
                results_metadata.append(qry_set.filter(record_id=record_id).values().first())
