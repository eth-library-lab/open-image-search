...
def download_lines(lines, out_dir, met_csv):
    ...
    with open(os.path.join(out_dir, "piece_info.csv"), 'w') as csv_file:
        ...
        for line in lines:
            ...
            try:
                driver.get(
                    'http://www.metmuseum.org/art/collection/search/' + line[4].strip())
                # 'http://www.metmuseum.org/art/collection/search/' + line[3].strip())
            ...
            try:
                ...
                #image_file = urllib3.HTTPConnectionPool.urlopen(image_link)
